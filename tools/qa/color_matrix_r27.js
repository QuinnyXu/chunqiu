/* 经纬春秋 · r27：第 10 国色「吴」判据全矩阵实算
 *
 * 用途：吴国色新立后，对「十国色 ＋ 全部功能色」做 ΔE76 全矩阵复验，
 *       并实测双底线（对绢帛 #F4EDDF ≥3.5、对白 #FFFFFF ≥4.1）、对功能色逐对核。
 *
 * 口径与 design_notes §2.1 / §2.4 / qi_palette_expansion.md §1.1 一字不差（沿用
 * tools/qa/color_matrix_r24a2.js 的同一套色彩数学，本脚本只扩色目与判据报表）：
 *   - ΔE76 = CIE Lab（D65/2°）欧氏距离；
 *   - 对比度 = WCAG 2.x 相对亮度比 (L1+0.05)/(L2+0.05)；
 *   - 灰阶 = sRGB→线性→WCAG Y→回伽马 ×255。
 * 判据（裁定 3，r27 由九色扩为十色）：十国色两两 ΔE76 ≥13.2；对绢帛 ≥3.5；对白 ≥4.1。
 * 对功能色：**逐对核并报数**——既有四对（郑×青绿 4.39／齐×朱砂 8.14／秦×玄墨 8.83／
 * 鲁×暖赭 9.21）系 r24a-2 裁定的观察项，本轮不动；**新立之色不得新增任何一对低于 13.2**。
 *
 * 色值一律**从 site/styles.css 与 site/app.js 实读**，脚本内不另抄一份。
 *
 * 用法：
 *   node tools/qa/color_matrix_r27.js            全矩阵复验（默认）
 *   node tools/qa/color_matrix_r27.js --search   第 10 色候选搜索（取色时用）
 */
"use strict";
const fs = require("fs");
...
function report() {
  console.log("=== r27 第 10 国色·吴 判据全矩阵（判据：两两 ΔE76 ≥" + T_DE +
  ...
  console.log("\n【二】十国色两两 ΔE76 全矩阵（" + (STATES.length * (STATES.length - 1) / 2) + " 对）");
  ...
  console.log("\n【四】十国色 × 全部非国色 " + NON_STATE.length + " 项，逐对核（只列 < " + T_DE + " 者，并注既有/新增）");
  const under = [];
  for (const s of STATES) {
...
/* ---------- 第 10 色候选搜索 ---------- */
function search() {
  const base = STATES.filter(s => s.name !== "吴");
  const targets = [...base, ...NON_STATE];
  const cand = [];
  for (let r = 8; r <= 210; r += 3) for (let g = 8; g <= 210; g += 3) for (let b = 8; b <= 210; b += 3) {
    const hex = rgb2hex([r, g, b]);
    if (contrast(hex, SILK) < T_SILK || contrast(hex, WHITE) < T_WHITE) continue;
    const gy = gray(hex);
    if (gy < 45 || gy > 125) continue;            // §2.2 可用灰阶带 [45,125]
    ...
    if (minState < T_DE) continue;                 // 硬判据：对九国色
