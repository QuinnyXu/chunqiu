/* 经纬春秋 · r28 徽记撞形实测（Vision）
 *
 * 沿用 r26b 立下的口径（design_notes §2.5.1），一字未改，只把名录扩到 33 枚、
 * NEW 换成本轮两枚（夫差·庭中之表／勾践·甲楯）：
 ...
 *
 * 跑法：node tools/qa/badge_silhouette_r28.js
 */
const fs = require("fs"), path = require("path");
const ICONS = path.resolve(__dirname, "..", "..", "site", "assets", "icons");
const OUT = path.resolve(__dirname, "screenshots");

const NEW = ["badge_fucha", "badge_goujian"];
const NAMES = {
  ...
  badge_fucha: "夫差·庭中之表（新）", badge_goujian: "勾践·甲楯（新）",
};
...
(async () => {
  ...
  for (const sc of SCALES) {
    ...
    /* 点名逐对（r28 本批最须自证可分的几组）：
     *  夫差×阖庐——父子同国同色，撞形代价最高的一对；
     *  夫差×武姜「掘地及泉」／叔向「昧旦」／伍员「耒」——同有一道横地线；
     *  夫差×宣姜「新台临河」——同为「地上起一构筑」的构图；
     *  夫差×子产「壞館之垣」——同为墙垣柱列一类的直线构筑；
     *  勾践×秦穆公「崤函之险」——「保于会稽」原拟连山形，实测后是否仍近（本枚已弃山只取楯）；
     *  勾践×骊姬「觚·鸩」／宋襄公「泓上之旆」／息妫「桃花」——同为竖长闭合廓形；
     *  勾践×夫差——宿敌并观时两枚同屏（并观左右轨各缀一枚小签），须一眼可分。 */
    const NAMED = [["badge_fucha", "badge_helu"], ["badge_fucha", "badge_wujiang"],
                   ["badge_fucha", "badge_shuxiang"], ["badge_fucha", "badge_wuyuan"],
                   ["badge_fucha", "badge_xuanjiang"], ["badge_fucha", "badge_zichan"],
                   ["badge_goujian", "badge_qinmu"], ["badge_goujian", "badge_liji"],
                   ["badge_goujian", "badge_songxiang"], ["badge_goujian", "badge_xigui"],
                   ["badge_fucha", "badge_goujian"]];
    console.log("\n  —— 点名逐对 ——");
    ...
    /* 同弧邻座（同国者同色，撞形代价最高）：齐 7、晋 4、鲁 4、郑 5、楚 3、吴 3、越 1（无组内对） */
    const GRP = {
  ...
  for (const sc of SCALES) {
    ...
    await p.setContent(`<body style="margin:0;background:#F4EDDF;font:12px/1.6 system-ui,sans-serif;color:#2E2A24">
      <div style="padding:14px 16px;font-size:14px">徽记 silhouette 对照 · ${sc.label} · 剥外圈 · r28（33 枚）</div>
    ...
    await p.screenshot({ path: path.join(OUT, "r28_badges_" + sc.size + "px.png"), fullPage: true });
  }
  console.log("\n对照图：screenshots/r28_badges_24px.png、r28_badges_22px.png");
  await br.close();
