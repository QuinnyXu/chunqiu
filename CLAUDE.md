...
- 排程与裁定权在领队与站长：执行中产生的改排建议、史学疑点，一律记录上报，不自行裁量（详见 docs/conventions.md §10）。
- 提交信息与交付文档使用团队轮次编号（delivery_<agent>_r<轮次>.md）。

## 任务书纪律（活文档原则）

1. **任务书（team/roundN_prompts.md）是唯一权威的活文档**：与站长及执行团队交流中产生的裁定、勘误、转性、作废、归并，一律**即时回写**任务书并标注日期——聊天粘贴块只是投递方式，不是存档。
2. **先入文、后发件**：执行块先写进任务书，再从任务书粘贴分发；不允许聊天先行、文件补记。
3. **写前查库**：凡任务书内容系于仓库现状（行文本、ID 尾号、已合入与否），落笔前先核实际状态，不凭记忆、不凭复述。
4. 执行者以任务书**现行文本**为准；发现任务书与实况不符，照"拿不准即停"红线上报，不自行取舍。
5. 作废与归并的任务原文**存档不删**，加注记录去向——账本要能看见拐过的弯。
...
4. 不删除他人产出；private/、team/、.claude/、wenjiang_research/ 等 .gitignore 条目永不入公开仓库。
   **附则二条**（2026-09-09 领队裁定二·九项辅助产物删除案立规，见 team/round49_prompts.md :113）：
   ① 他人未跟踪产物一律不 `rm`——移入带日期之 `_to_delete/` 并于交付文档留清单；**该目录一律置于 `team/` 下，不得置于仓库根**（2026-09-20 站长裁二十二核可，理由句见 docs/conventions.md §10.3）；
