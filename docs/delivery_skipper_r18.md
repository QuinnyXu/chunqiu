...
## 8. 补记：收款码换图上线（2026-07-26 深夜，续 r18，§7 异常项裁定落地）

**裁定**：Xu 明确裁定，工作区那张 88209 字节版本（仅二维码＋灰色头像占位、无收款人昵称文字）**就是要上线的新收款码**，即 §7 上报的"异常"实为待落地的正确改动，非误操作。

**复核**：合入前重新核对工作区 `site/assets/support/alipay-qr.png` 仍为 88209 字节、`git status` 仅此一个文件变动，无夹带其他改动。
...
**提交/部署**：
- Commit `61e8b4b`（chore(site): r18 同名替换收款码图上线，`alipay-qr.png` 136844→88209 字节）
- Push → GitHub Actions run **`30230362702`，headSha `61e8b4b`，success**（`gh run watch` 阻塞确认，全部 job 步骤绿）

**关键核验（生产已确认换成新图，非缓存旧图）**：
- `curl -I https://chunqiu.timechorus.com/assets/support/alipay-qr.png` → `200 OK`，`Content-Length: 88209`（非旧图的 136844）
- 逐字节下载比对：生产文件与本地仓库文件 **SHA256 完全一致**（`2adac64a21b6194b4c0636fca521a5327c6aaadd217c393b38191489cde28026`）
- `meta.json.generated_at` 本地/线上一致（`2026-07-21T12:31:04+00:00`），`OK：线上 meta.json 与仓库一致，部署已生效`

§7 记录的异常项至此闭环：疑点已按流程上报、未擅自处置，经 Xu 裁定后按裁定结果合入部署，生产站现已是裁定版本。
...
- ~~⚠ 待裁定：工作区存在未纳入任何提交的 `alipay-qr.png` 改动~~ **已裁定闭环**：Xu 确认 88209 字节版即应上线新图，已提交 `61e8b4b` 并部署（run `30230362702` success），生产字节数/SHA256 均已核验一致，详见 §8。
