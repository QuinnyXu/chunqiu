 ...
 * r32 补第 6 档「出土文献」（J 前缀，清华简《系年》等），专属窖土褐 --excav——
 * 未补此键之前，该 quote_type 落在下方 `|| ""` 的空串回退上，与「原文」同呈现（朱线朱底），
 * 新开一层却与骨架层撞色。回退分支本身不动：未知层仍是不加徽标、不报错。 */
const QLAYER_CLASS = {
  "言论": "layer-yanlun",
  "评论": "layer-pinglun",
  "后出叙事": "layer-houchu",
 ...
 * 视觉上用暖赭（界面/编者语态），与史料层色（朱=经传、菉=诗歌、紫=经义异闻、灰=后出叙事、
 * 窖土褐=出土文献）区分——Q442（清华简《系年》）正是同卡并置层徽标与编者层标的实例，
 * 二者 ΔE76 30.51，编者的话与史料本身一眼可分：
...
function eventQuotesFrag(evt) {
  ...
  for (const q of DATA.passages.filter(p => p.event_id === evt.id)) {
    ...
    // 经义异闻（公羊/穀梁传注异说）、后出叙事（史记等晚出戏剧化）、诗歌（诗经舆论层）、
    // 出土文献（J 层，清华简等，r32 新立）、言论、评论。
